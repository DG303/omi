...
<AccordionGroup>
  <Accordion title="Query Parameters" icon="sliders" defaultOpen={true}>
    ...
    | `categories` | string | - | Comma-separated list of categories to filter by (e.g. `work,personal`) |
    | `folder_id` | string | - | Filter by folder ID. Must be a non-empty string if provided. Use `GET /v1/dev/user/folders` to retrieve folder IDs. |
    | `starred` | boolean | - | Filter by starred status. `true` for starred only, `false` for non-starred only. Omit to return all. |
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Transcript Segment Fields" icon="waveform-lines">
    | Field | Type | Description |
    |-------|------|-------------|
    | `id` | string | Segment identifier (optional) |
    | `text` | string | The transcribed text |
    | `speaker_id` | integer | Numeric speaker identifier (optional) |
    | `speaker_name` | string | Human-readable speaker name. Resolved from user profiles or the People database. Falls back to a generic name like "Speaker 1" if unidentified. (optional) |
